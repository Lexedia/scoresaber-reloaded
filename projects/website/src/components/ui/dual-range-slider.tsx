...
export function DualRangeSlider({
  ...
  const currentValue = Array.isArray(value) && value.length === 2 ? value : [
    min,
    max,
  ]
  const [
      ...
      {currentValue.map((val, index) => (
        ...
        >
          {showLabel && (
            <span
              className={cn(
                `bg-primary/10 text-primary pointer-events-none absolute flex w-fit min-w-8 items-center justify-center
                rounded-md px-1.5 py-0.5 text-sm font-medium select-none`,
