...
function AverageCutValues({ cuts, hand }: {
  cuts: number[];
  hand: Hand
}) {
  return (
...
function AccuracyCircle({ accuracy, averageCut, hand }: AccuracyCircleProps) {
  ...
  return (
    <div className="flex items-center gap-2 md:gap-3">
      <div
        className={cn('flex items-center gap-2 md:gap-3', hand === 'right' ? 'flex-row-reverse' : 'flex-row')}
      >
...
function HandStat({ hand, name, value }: {
  hand: Hand;
  name: string;
  value: string
}) {
...
function HandAccuracy({ scoreStats, hand }: {
  scoreStats: ScoreStatsToken;
  hand: Hand
}) {
  const {
    accuracy, averageCut, timeDependence, preSwing, postSwing,
  } = useHandStats(scoreStats, hand)
