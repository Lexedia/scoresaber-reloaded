...
function getBestAccuracyAndStarsForOnePp(sortedPps: number[]): {
  accuracy: number;
  stars: number
} {
...
function RawPpDisplay({ label, value, onReset }: {
  label: string;
  value: string;
  onReset: () => void
}) {
...
function AccuracyThresholdTable({ targetRawPp, showDashes }: {
  targetRawPp: number;
  showDashes: boolean
}) {
  return (
...
export default function PlusPpCalculator({ player }: { player: ScoreSaberPlayer }) {
  ...
  const rawPpFromPlay = useMemo(() => ScoreSaberCurve.getPp(stars, accuracy), [
    stars,
    accuracy,
  ])
