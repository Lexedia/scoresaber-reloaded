...
export class ReplayDecoder {
  ...
  private static ParseKnownCustomData(replay: Replay): void {
    ...
    if (replay.customData['HeartBeatQuest']) {
      ...
      if (version === 1) {
        ...
        for (let i = 0; i < len; i++) {
          frames[i] = {
            time: this.ReadFloat(hbState),
            heartrate: this.ReadInt(hbState),
          }
        }
        replay.parsedCustomData['HeartBeatQuest'] = {
          frames,
          device: this.ReadString(hbState),
        }
      }
    }
  ...
  private static DecodeEuler(state: DecodeState): Euler {
    return {
      position: this.DecodeVector3(state),
      rotation: this.DecodeQuaternion(state),
    }
  }
