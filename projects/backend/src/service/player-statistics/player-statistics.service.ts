...
export class PlayerStatisticsService {
  ...
  public static async getStatistics(playerToken: ScoreSaberPlayerToken): Promise<ScoreSaberPlayerStatistics> {
    ...
    return {
      ...
      medianRankedAccuracy: scoreStats.medianRankedAccuracy,
      medianUnrankedAccuracy: scoreStats.medianUnrankedAccuracy,
      medianAccuracy: scoreStats.medianAccuracy,
  ...
  private static async getScoreStats(playerId: string) {
    return CacheService.fetch(CacheId.PLAYER_SCORE_STATISTICS, playerId, async () => ScoreSaberScoresRepository.getPlayerScoreStatistics(playerId))
  }
}
