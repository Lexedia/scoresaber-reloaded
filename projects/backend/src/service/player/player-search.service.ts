...
export class PlayerSearchService {
  ...
  public static async searchPlayers(query?: string): Promise<ScoreSaberPlayer[]> {
    ...
    const scoreSaberPlayerTokenMap = new Map((scoreSaberPlayerTokens ?? []).map(token => [
      token.id,
      token,
    ]))
    const uniquePlayerIds = [ ...new Set(localMatches.map(p => p.id).concat(scoreSaberPlayerTokens?.map(token => token.id) ?? [])) ]
