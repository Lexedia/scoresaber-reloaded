...
export const app = new Elysia()
  ...
  .use(
    cron({
      ...
      pattern: '0 3 * * * *', // Every day at 3:00 utc
      timezone: 'UTC',
      protect: true,
      run: async () => {
  ...
  .onError({ as: 'global' }, ({ code, error }) => {
     ...
     * JSON round-trip drops Elysia class instances (e.g. RequestParams in `value`) so mapResponse's devalue can serialize.
     */
    if (code === 'VALIDATION') {
  ...
  .onAfterHandle(async ({
    request, route, response, set,
  }) => {
    await httpMetricsHooks.onAfterHandle({
      request,
      route,
      response,
      set,
